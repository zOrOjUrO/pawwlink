...
export default function OwnerRegisterPage() {
  ...
  async function submit() {
    if (!file) {
      setError("Please add a photo of your pet.");
      return;
    }
    setSubmitting(true);
    setError(null);
  ...
  if (ownerId) {
    return (
      <main className="max-w-md mx-auto px-4 py-16 text-center">
        <div className="h-16 w-16 mx-auto rounded-full bg-meadow/15 flex items-center justify-center text-meadow text-3xl">✓</div>
        ...
        <div className="mt-8 flex flex-col gap-3">
          <Link href="/owner/search" className="rounded-xl bg-rescue text-white font-display font-semibold py-3.5">
            Browse found animals
          </Link>
          <Link href="/" className="rounded-xl border border-mist bg-white text-slate-brand font-display font-semibold py-3.5">
            Done
          </Link>
        </div>
      </main>
  ...
  return (
    <main className="max-w-md mx-auto px-4 py-8">
      ...
      <div className="flex items-center gap-2 mb-6">
        {[1, 2, 3].map((s) => (
          <div key={s} className={`h-1.5 flex-1 rounded-full ${s <= step ? "bg-rescue" : "bg-mist"}`} />
        ))}
      </div>
      ...
      {error && (
        <div className="mb-4 rounded-xl border border-signal/40 bg-signal/10 text-signal px-4 py-3 text-sm">{error}</div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <button
            onClick={() => (name.trim() ? setStep(2) : setError("Please enter your name."))}
            className="w-full rounded-xl bg-rescue text-white font-display font-semibold py-3.5 hover:brightness-95"
          >
            Continue
          </button>
        </div>
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setStep(3)} className="flex-1 rounded-xl bg-rescue text-white font-display font-semibold py-3.5 hover:brightness-95">Continue</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 rounded-xl bg-rescue text-white font-display font-semibold py-3.5 hover:brightness-95 disabled:opacity-40"
            >
