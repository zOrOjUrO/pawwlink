...
export default function SyncPage() {
  ...
  const retry = useCallback(
    async (item: QueuedIntake) => {
      setBusy(true);
      try { await syncOne(item); markSynced(item.id); } catch { markFailed(item.id); }
  ...
  const syncAll = useCallback(async () => {
    ...
    for (const it of getQueue().filter((i) => i.status !== "synced")) {
      try { await syncOne(it); markSynced(it.id); } catch { markFailed(it.id); }
    }
    refresh();
    setBusy(false);
  }, [refresh]);
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between gap-4 mb-6">
        ...
        <button onClick={syncAll} disabled={busy || pending === 0} className="rounded-xl bg-rescue text-white font-display font-semibold px-4 py-2.5 transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-40">
        ...
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 rounded-2xl border border-mist bg-white p-3 shadow-sm">
              ...
              <span className={`rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${STATUS_CLS[item.status]}`}>{item.status}</span>
              {item.status !== "synced" && (
                <button onClick={() => retry(item)} disabled={busy} className="rounded-lg border border-mist px-3 py-1.5 text-sm font-medium text-slate-brand hover:bg-cloud disabled:opacity-40">Retry</button>
              )}
            </div>
          ))}
        </div>
