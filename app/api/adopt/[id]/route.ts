...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await insertAdopter({ animal_id: id, name: body.adopter_name, phone: body.adopter_phone, email: body.adopter_email });
    try { await updateAnimal(id, { status: "matched" }); } catch { /* best-effort */ }
    return NextResponse.json({ success: true, message: "Shelter will contact you within 24 hours" });
  } catch (err) {
